...
/// One macFUSE + mtpfuse process per USB location (supports several phones at once). Pure MTP (libmtp).
final class MountManager {
    enum MountError: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .helperMissing:
                return "FUSE helper mtpfuse not found. Run `make bundle` from the project."
    ...
    func adoptOrphanMountsIfNeeded() -> [(locationID: UInt32, displayName: String)] {
        ...
        for name in entries {
            ...
            if !verifyBrowsableVolume(at: path, quick: true) {
    ...
    private static func fuseVolumeLabel(deviceName: String) -> String {
        let trimmed = deviceName.trimmingCharacters(in: .whitespacesAndNewlines)
        return sanitizeVolumeLabel(trimmed)
    }
    ...
    private func startFuseMount(
        ...
        guard let helperPath = resolvedMtpfusePath() else {
        ...
        do {
            ...
            DispatchQueue.global().async { [weak self] in
                ...
                for _ in 0..<40 {
                    ...
                    if self.isFuseMounted(at: mountPoint) {
                        ...
                        self.sessions[device.locationID] =
                            Session(mountPoint: mountPoint, process: p)
                ...
                if !errTail.isEmpty {
                    ...
                    detail += " \(helperTag) exited; check MTP mode and USB connection."
                }
    ...
    /// Wait until the FUSE root lists MTP storage volumes.
    /// - Parameter quick: Shorter polling when adopting orphans at launch (avoid blocking UI).
    private func verifyBrowsableVolume(at mountPoint: String, quick: Bool = false) -> Bool {
        let fm = FileManager.default
        let ignored = Set([
            ".metadata_never_index", ".DS_Store",
        ])
        let attempts = quick ? 18 : 70
        let step: TimeInterval = quick ? 0.1 : 0.2
        for i in 0..<attempts {
            if i > 0 {
    ...
    private func resolvedMtpfusePath() -> String? {
        ...
        candidates.append(FileManager.default.currentDirectoryPath + "/build/mtpfuse")
